  ...
  const fetchMaintenanceData = async () => {
    ...
    try {
      const { data: workOrders, error: workOrdersError } = await supabase
        .from('work_orders')
        .select(`*, equipment:equipment_id (id, name, code), assigned_to_profile:assigned_to (full_name)`)
      ...
      const { data: scheduledPlans, error: schedulesError } = await supabase
        ...
        .select(`*, maintenance_plans:maintenance_plan_id (name, type, priority, description, tasks, frequency_days, end_date, equipment:equipment_id (id, name))`)
