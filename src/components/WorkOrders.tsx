...
// import { Plus, Search, Filter, Clock, CheckCircle, AlertTriangle, User, Wrench, Calendar, Edit, Trash2, XCircle, Printer, Package } from "lucide-react";
...
//     used_resources: [] as string[], // NOVO: Estado para recursos usados
//   });
//   const [newTaskInput, setNewTaskInput] = useState("");
//   const [newUsedResourceInput, setNewUsedResourceInput] = useState(""); // NOVO: Estado para input de recurso usado
...
//       // Lógica para desserializar tasks do plano (se vierem como string JSON)
//       let prefillTasks: string[] = [];
//       if (prefillData.tasks && typeof prefillData.tasks === 'string') {
//         try {
//           prefillTasks = JSON.parse(prefillData.tasks);
//         } catch (e) {
//           console.error("Falha ao desserializar tarefas no prefill:", e);
//           prefillTasks = [];
//         }
//       } else if (Array.isArray(prefillData.tasks)) {
//         prefillTasks = prefillData.tasks;
//       }

//       // Lógica para desserializar required_resources do plano (se vierem como string JSON)
//       let prefillResources: string[] = [];
//       if (prefillData.required_resources && typeof prefillData.required_resources === 'string') {
//         try {
//           prefillResources = JSON.parse(prefillData.required_resources);
//         } catch (e) {
//           console.error("Falha ao desserializar recursos no prefill:", e);
//           prefillResources = [];
//         }
//       } else if (Array.isArray(prefillData.required_resources)) {
//         prefillResources = prefillData.required_resources;
//       }
...
//         tasks: prefillTasks,
//         used_resources: prefillResources, // NOVO: Preenche com recursos do plano
...
//   // NOVO: Funções para gerenciar recursos utilizados
//   const handleAddUsedResource = () => {
//     if (newUsedResourceInput.trim() !== "") {
//       setNewOrder(prev => ({
//         ...prev,
//         used_resources: [...prev.used_resources, newUsedResourceInput.trim()]
//       }));
//       setNewUsedResourceInput("");
//     }
//   };

//   const handleRemoveUsedResource = (indexToRemove: number) => {
//     setNewOrder(prev => ({
//       ...prev,
...
//       used_resources: newOrder.used_resources.length > 0 ? newOrder.used_resources : null, // NOVO: Inclui recursos
//     };
...
//       setNewOrder({
//         title: "", type: "", description: "", priority: "medium", estimated_hours: "",
//         scheduled_date: "", equipment_id: "", maintenance_plan_id: "", tasks: [],
//         used_resources: [], // NOVO: Resetar recursos também
//       });
//       setNewTaskInput("");
//       setNewUsedResourceInput(""); // NOVO: Resetar input de recurso usado
//     }
//   };

//   const handleDeleteOrder = async (orderId: string) => { if (confirm("Tem certeza que deseja deletar esta ordem?")) { await deleteWorkOrder(orderId); } };
...
//   const filteredOrders = workOrders.filter(order => { const matchesSearch = order.title.toLowerCase().includes(searchTerm.toLowerCase()) || (order.equipment?.name || "").toLowerCase().includes(searchTerm.toLowerCase()); const matchesStatus = selectedStatus === "all" || order.status === selectedStatus; if (activeTab === "active") return matchesSearch && matchesStatus && !["completed", "cancelled"].includes(order.status); else if (activeTab === "completed") return matchesSearch && matchesStatus && order.status === "completed"; else return matchesSearch && matchesStatus && order.status === "cancelled"; });
...
//         <PrintableWorkOrder order={orderToPrint} />
//       </div>
...
//                                 <Input id="order-title" placeholder="Ex: Manutenção Preventiva - Equipamento" value={newOrder.title} onChange={(e) => setNewOrder({...newOrder, title: e.target.value})}/>
//                             </div>
//                             <div className="space-y-2">
...
//                                 <Select value={newOrder.equipment_id} onValueChange={(value) => setNewOrder({...newOrder, equipment_id: value})}>
//                                     <SelectTrigger><SelectValue placeholder="Selecione o equipamento" /></SelectTrigger>
//                                     <SelectContent>
//                                         {equipment.map(eq => (<SelectItem key={eq.id} value={eq.id}>{eq.name} ({eq.code})</SelectItem>))}
//                                     </SelectContent>
//                                 </Select>
//                             </div>
//                             <div className="space-y-2">
//                                 <Label htmlFor="priority">Prioridade</Label>
//                                 <Select value={newOrder.priority} onValueChange={(value) => setNewOrder({...newOrder, priority: value})}><SelectTrigger><SelectValue placeholder="Selecione a prioridade" /></SelectTrigger><SelectContent><SelectItem value="critical">Crítica</SelectItem><SelectItem value="high">Alta</SelectItem><SelectItem value="medium">Média</SelectItem><SelectItem value="low">Baixa</SelectItem></SelectContent></Select>
//                             </div>
//                             <div className="space-y-2">
//                                 <Label htmlFor="estimated-hours">Horas Estimadas</Label>
//                                 <Input id="estimated-hours" type="number" placeholder="Ex: 4" value={newOrder.estimated_hours} onChange={(e) => setNewOrder({...newOrder, estimated_hours: e.target.value})}/>
//                             </div>
//                             <div className="space-y-2">
//                                 <Label htmlFor="scheduled-date">Data Agendada</Label>
//                                 <Input id="scheduled-date" type="date" value={newOrder.scheduled_date} onChange={(e) => setNewOrder({...newOrder, scheduled_date: e.target.value})}/>
//                             </div>
//                             <div className="col-span-2 space-y-2">
//                                 <Label htmlFor="description">Descrição Adicional</Label>
//                                 <Textarea id="description" placeholder="Adicione observações ou detalhes extras..." value={newOrder.description} onChange={(e) => setNewOrder({...newOrder, description: e.target.value})}/>
//                             </div>
//                             <div className="col-span-2 space-y-2">
//                                 <Label htmlFor="tasks">Tarefas da Ordem de Serviço</Label>
//                                 <div className="space-y-2">
//                                     {Array.isArray(newOrder.tasks) && newOrder.tasks.map((task, index) => (
//                                         <div key={index} className="flex items-center justify-between bg-gray-100 p-2 rounded-md">
//                                             <span className="text-sm">{task}</span>
//                                             <Button type="button" variant="ghost" size="sm" onClick={() => handleRemoveTask(index)}><XCircle className="w-4 h-4 text-red-500" /></Button>
//                                         </div>
//                                     ))}
//                                     {(!newOrder.tasks || newOrder.tasks.length === 0) && (
//                                         <p className="text-sm text-muted-foreground">Nenhuma tarefa herdada do plano.</p>
//                                     )}
//                                     <div className="flex space-x-2 pt-2">
//                                         <Input id="new-task-input" placeholder="Adicionar nova tarefa" value={newTaskInput} onChange={(e) => setNewTaskInput(e.target.value)} onKeyPress={(e) => { if (e.key === 'Enter') { e.preventDefault(); handleAddTask(); } }}/>
...
//                             <div className="col-span-2 space-y-2">
//                                 <Label htmlFor="used-resources">Recursos Utilizados</Label>
//                                 <div className="flex space-x-2">
//                                     <Input
//                                         id="new-used-resource-input"
//                                         placeholder="Adicionar recurso utilizado (Ex: Parafuso M8)"
//                                         value={newUsedResourceInput}
//                                         onChange={(e) => setNewUsedResourceInput(e.target.value)}
//                                         onKeyPress={(e) => {
//                                             if (e.key === 'Enter') {
//                                                 e.preventDefault();
//                                                 handleAddUsedResource();
//                                             }
//                                         }}
//                                     />
//                                     <Button type="button" onClick={handleAddUsedResource}>Adicionar</Button>
//                                 </div>
//                                 <div className="space-y-1">
//                                     {newOrder.used_resources.map((resource, index) => (
//                                         <div key={index} className="flex items-center justify-between bg-gray-100 p-2 rounded-md">
...
//                                                 variant="ghost"
//                                                 size="sm"
//                                                 onClick={() => handleRemoveUsedResource(index)}
//                                             >
...
//                                     ))}
//                                 </div>
//                                 {newOrder.used_resources.length === 0 && (
...
//                     {/* NOVO: Exibir Recursos Utilizados */}
...
//                         {order.used_resources.map((resource, idx) => (
...
//                       </div>
//                     )}
//                     {/* FIM NOVO: Exibir Recursos Utilizados */}
//                     {order.description && (
...
//                         <Button variant="outline" size="sm" className="flex-1" onClick={() => setAssigningOrderId(order.id)}><User className="w-4 h-4 mr-1" />{order.assigned_to ? 'Reatribuir' : 'Atribuir'}</Button>
//                         <Button variant="outline" size="sm" className="flex-1" onClick={() => setRescheduleOrderId(order.id)}><Calendar className="w-4 h-4 mr-1" />Reagendar</Button>
//                         <Button variant="outline" size="sm" className="flex-1" onClick={() => setExecutingOrder(order)} disabled={order.status === 'completed'}>
...
//             createFollowUpOrder={createWorkOrder} // Passamos a função de criar OS
//         />        
...
import { Textarea } from "@/components/ui/textarea";
import { Tabs, TabsContent, TabsList, TabsTrigger } from "@/components/ui/tabs";
import { Plus, Search, Filter, Clock, CheckCircle, AlertTriangle, User, Wrench, Calendar, Edit, Trash2, XCircle, Printer } from "lucide-react";
import { useWorkOrders, WorkOrder } from "@/hooks/useWorkOrders";
import { toast } from "@/hooks/use-toast";
import WorkOrderEditDialog from "./WorkOrderEditDialog";
import WorkOrderAssignDialog from "./WorkOrderAssignDialog";
...
const WorkOrders = ({ prefillData, onPrefillHandled }: WorkOrdersProps) => {
  ...
  const filteredOrders = workOrders.filter(order => {
    ...
    return true; // Fallback para casos inesperados
  });
