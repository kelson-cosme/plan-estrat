...
const WorkOrders = ({ prefillData, onPrefillHandled }: WorkOrdersProps) => {
  ...
  // Efeito para lidar com os dados pré-preenchidos
  useEffect(() => {
  ...
  const handleCreateOrder = async () => {
    if (!newOrder.title || !newOrder.type) {
      toast({ title: "Erro", description: "Título e tipo são obrigatórios", variant: "destructive" });
      return;
    }
    ...
    const orderData = {
      ...
      equipment_id: newOrder.equipment_id || undefined, // USANDO
      maintenance_plan_id: newOrder.maintenance_plan_id || undefined, // USANDO
    };
    ...
    if (result) {
      ...
      setNewOrder({
        ...
        scheduled_date: "", equipment_id: "", maintenance_plan_id: "",
      });
    }
  };
  ...
  const handleAssignOrder = async (orderId: string, assignedTo: string) => {
    return await updateWorkOrder(orderId, { assigned_to: assignedTo, status: 'in_progress' });
  };
  ...
  const handleRescheduleOrder = async () => {
    if (!rescheduleOrderId || !rescheduleDate) {
      toast({ title: "Erro", description: "Selecione uma nova data", variant: "destructive" });
      return;
    }
    const result = await updateWorkOrder(rescheduleOrderId, { scheduled_date: rescheduleDate });
    if (result) {
      setRescheduleOrderId(null);
      setRescheduleDate("");
      toast({ title: "Ordem reagendada", description: "Data atualizada com sucesso" });
    }
  };
  ...
  const filteredOrders = workOrders.filter(order => {
    ...
    else if (activeTab === "completed") return matchesSearch && matchesStatus && order.status === "completed";
    else return matchesSearch && matchesStatus && order.status === "cancelled";
  });
  ...
  if (loading) {
    return (<div className="flex items-center justify-center min-h-64"><div className="text-center"><div className="animate-spin rounded-full h-8 w-8 border-b-2 border-blue-600 mb-2 mx-auto"></div><p>Carregando ordens...</p></div></div>);
  }
  ...
  return (
    <div className="space-y-6">
            ...
            <Dialog open={isCreateDialogOpen} onOpenChange={setIsCreateDialogOpen}><DialogTrigger asChild><Button><Plus className="w-4 h-4 mr-2" />Nova Ordem</Button></DialogTrigger>
              <DialogContent className="max-w-3xl">
                ...
                <div className="grid grid-cols-2 gap-4 py-4">
                  ...
                  <div className="col-span-2 space-y-2"><Label htmlFor="description">Descrição dos Serviços</Label><Textarea id="description" placeholder="Descreva detalhadamente os serviços a serem executados..." value={newOrder.description} onChange={(e) => setNewOrder({...newOrder, description: e.target.value})}/></div>
                </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        <Card><CardContent className="p-6"><div className="flex items-center justify-between"><div><p className="text-sm font-medium text-gray-600">Ordens Ativas</p><p className="text-2xl font-bold">{activeOrders.length}</p></div><div className="h-12 w-12 bg-blue-100 rounded-lg flex items-center justify-center"><Clock className="h-6 w-6 text-blue-600" /></div></div></CardContent></Card>
        <Card><CardContent className="p-6"><div className="flex items-center justify-between"><div><p className="text-sm font-medium text-gray-600">Concluídas</p><p className="text-2xl font-bold">{completedOrders.length}</p></div><div className="h-12 w-12 bg-green-100 rounded-lg flex items-center justify-center"><CheckCircle className="h-6 w-6 text-green-600" /></div></div></CardContent></Card>
        <Card><CardContent className="p-6"><div className="flex items-center justify-between"><div><p className="text-sm font-medium text-gray-600">Canceladas</p><p className="text-2xl font-bold">{cancelledOrders.length}</p></div><div className="h-12 w-12 bg-red-100 rounded-lg flex items-center justify-center"><AlertTriangle className="h-6 w-6 text-red-600" /></div></div></CardContent></Card>
      </div>
