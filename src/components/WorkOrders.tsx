...
import { Plus, Search, Filter, Clock, CheckCircle, AlertTriangle, User, Wrench, Calendar, Edit, Trash2, XCircle, Printer } from "lucide-react"; // Importar Printer
...
import { PrintableWorkOrder } from "./PrintableWorkOrder"; // Importar o componente de impressão
...
const WorkOrders = ({ prefillData, onPrefillHandled }: WorkOrdersProps) => {
  ...
  const [orderToPrint, setOrderToPrint] = useState<WorkOrder | null>(null); // Estado para a ordem a ser impressa
  ...
  useEffect(() => {
    if (prefillData && onPrefillHandled) {
      setNewOrder({
        ...
        tasks: Array.isArray(prefillData.tasks) ? prefillData.tasks : [],
        estimated_hours: "",
      });
  ...
  const handlePrint = (order: WorkOrder) => {
    ...
    setTimeout(() => {
      window.print();
      setOrderToPrint(null); // Limpa o estado após a impressão
    }, 100); // Pequeno delay para garantir a renderização do componente
  };
  ...
  const handleCreateOrder = async () => { if (!newOrder.title || !newOrder.type) { toast({ title: "Erro", description: "Título e tipo são obrigatórios", variant: "destructive" }); return; } const tasksString = newOrder.tasks.length > 0 ? `\n--- Tarefas ---\n${newOrder.tasks.map(task => `- ${task}`).join('\n')}` : ''; const finalDescription = `${newOrder.description}${tasksString}`; const orderData = { title: newOrder.title, type: newOrder.type as 'preventiva' | 'preditiva' | 'corretiva', description: finalDescription.trim() || undefined, priority: newOrder.priority as 'low' | 'medium' | 'high' | 'critical', estimated_hours: newOrder.estimated_hours ? parseInt(newOrder.estimated_hours) : undefined, scheduled_date: newOrder.scheduled_date || undefined, status: 'open' as const, equipment_id: newOrder.equipment_id || undefined, maintenance_plan_id: newOrder.maintenance_plan_id || undefined, }; const result = await createWorkOrder(orderData); if (result) { setIsCreateDialogOpen(false); setNewOrder({ title: "", type: "", description: "", priority: "medium", estimated_hours: "", scheduled_date: "", equipment_id: "", maintenance_plan_id: "", tasks: [], }); setNewTaskInput(""); } };
  ...
  return (
    <>
      {/* Componente de impressão (não visível na tela normal) */}
      ...
      <div className="space-y-6">
            ...
            <CardHeader>
                <div className="flex justify-between items-center">
                    ...
                    <DialogContent className="max-w-3xl">
                        ...
                        <div className="grid grid-cols-2 gap-4 py-4 max-h-[70vh] overflow-y-auto pr-4"></div>
                        ...
                        <div className="col-span-2 space-y-2">
                            ...
                            <div className="space-y-2">{Array.isArray(newOrder.tasks) && newOrder.tasks.map((task, index) => (<div key={index} className="flex items-center justify-between bg-gray-100 p-2 rounded-md"><span className="text-sm">{task}</span><Button type="button" variant="ghost" size="sm" onClick={() => handleRemoveTask(index)}><XCircle className="w-4 h-4 text-red-500" /></Button></div>))}{(!newOrder.tasks || newOrder.tasks.length === 0) && (<p className="text-sm text-muted-foreground">Nenhuma tarefa herdada do plano.</p>)}
                            <div className="flex space-x-2 pt-2"><Input id="new-task-input" placeholder="Adicionar nova tarefa" value={newTaskInput} onChange={(e) => setNewTaskInput(e.target.value)} onKeyPress={(e) => { if (e.key === 'Enter') { e.preventDefault(); handleAddTask(); } }}/><Button type="button" onClick={handleAddTask}>Adicionar</Button></div>
                        </div>
                        </div>
        ...
        {/* ... restante da UI (cards de resumo e abas) ... */}
