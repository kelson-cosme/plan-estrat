      ...
      <div className="space-y-6">
        ...
        <Tabs value={activeTab} onValueChange={setActiveTab}>
            ...
            {filteredOrders.length === 0 ? (<Card><CardContent className="text-center py-12"><Wrench className="w-12 h-12 text-gray-400 mx-auto mb-4" /><h3 className="text-lg font-medium text-gray-900 mb-2">Nenhuma ordem encontrada</h3><p className="text-gray-600">{workOrders.length === 0 ? "Crie uma nova ordem de serviço para começar." : "Tente ajustar seus filtros de busca ou crie uma nova ordem."}</p></CardContent></Card>
                    ...
                    {order.description && (<div className="pt-3 border-t"><p className="text-sm text-gray-600 mb-1">Descrição:</p><p className="text-sm whitespace-pre-wrap">{order.description}</p></div>)}<div className="flex space-x-2 pt-3"><Button variant="outline" size="sm" className="flex-1" onClick={() => setAssigningOrderId(order.id)}><User className="w-4 h-4 mr-1" />{order.assigned_to ? 'Reatribuir' : 'Atribuir'}</Button><Button variant="outline" size="sm" className="flex-1" onClick={() => setRescheduleOrderId(order.id)}><Calendar className="w-4 h-4 mr-1" />Reagendar</Button><Button variant="outline" size="sm" className="flex-1" onClick={() => setExecutingOrderId(order.id)} disabled={order.status === 'completed'}><Wrench className="w-4 h-4 mr-1" />Executar</Button></div></CardContent></Card>))}</div>)}
            </TabsContent>
        </Tabs>
        ...
        <WorkOrderExecuteDialog orderId={executingOrderId} isOpen={!!executingOrderId} onClose={() => setExecutingOrderId(null)} onExecute={handleExecuteOrder} />
