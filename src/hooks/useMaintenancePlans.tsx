// src/components/useMaintenancePlans.tsx
...
import { Plus, Calendar, Clock, Wrench, AlertTriangle, CheckCircle, Edit, Trash2, Play, Pause, Settings, XCircle, Package } from "lucide-react"; // Importar Package para recursos
...
import MaintenanceScheduler from "@/components/MaintenanceScheduler"; // Caminho ajustado
import MaintenancePlanEditDialog from "@/components/MaintenancePlanEditDialog"; // Caminho ajustado
...
const MaintenancePlans = () => {
  ...
  const [newPlan, setNewPlan] = useState({
    ...
    required_resources: [] as string[], // NOVO: Estado para recursos necessários
  });
  ...
  const handleCreatePlan = async () => {
    ...
    const planData = {
      ...
      required_resources: newPlan.required_resources.length > 0 ? newPlan.required_resources : null, // NOVO
      active: true
    };
    ...
    if (result) {
      ...
      setNewPlan({
        ...
        required_resources: [], // NOVO: Resetar recursos também
      });
      setNewTaskInput("");
      setNewResourceInput(""); // NOVO: Resetar input de recurso
    }
  };
  ...
  const handleExecutePlan = async (plan: MaintenancePlanType) => {
    const workOrderData = {
      ...
      used_resources: plan.required_resources, // NOVO: Transfere recursos necessários
    };
  ...
  const handleCreateScheduledOrder = async () => {
    ...
    const workOrderData = {
      ...
      used_resources: selectedPlanForSchedule.required_resources, // NOVO: Transfere recursos necessários
    };
              ...
              {filteredPlans.map((plan) => (
                <Card key={plan.id} className="hover:shadow-lg transition-shadow">
                  ...
                  <CardContent className="space-y-4">
                    ...
                    {/* NOVO: Exibir Recursos Necessários */}
                    ...
                    {/* FIM NOVO: Exibir Recursos Necessários */}
